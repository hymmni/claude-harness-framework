---
name: harness
description: 이 프로젝트의 개발 워크플로우. 새 구현 작업을 설계·계획·실행할 때, 또는 scripts/execute.py로 플랜을 실행할 때 사용한다.
---
...
`/docs/` 하위 문서(ROBOT_GUIDE, ARCHITECTURE, ADR 등)를 읽고 프로젝트의 아키텍처·설계 의도를 파악한다. 필요시 Explore 에이전트를 병렬로 사용한다.
...
task/플랜 완료처럼 의미 있는 작업 단위가 끝날 때마다 컨텍스트가 충분히 쌓였는지
스스로 판단해 `/compact`, `/clear`, 또는 새 세션 시작을 사용자에게 먼저 권장하라.
